...
export default {
  ...
  methods: {
    ...
    close() {
      // Method to close the page and make it disappear
      this.closeReport = false;
    },
  },
  mounted() {
    // Listen for the "report-sent" event emitted by the SenderComponent
    this.$root.$on("report-sent", this.receiveReport);
  },
  beforeDestroy() {
    // Remove the event listener when the component is destroyed
    this.$root.$off("report-sent", this.receiveReport);
  },
};
</script>
